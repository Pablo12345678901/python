...
class MainWidget(RelativeLayout):
    ...
    SPEED_X = .01 # pour la vitesse de décallage sur appui des touches / appui sur l'écran
    current_speed_x = 0
    current_offset_x = 0
    ...
    def reset_game(self):
        self.SPEED = 0.005 # remise à la valeur initiale car accélération y dans le jeu au fil du temps
        self.SPEED_X = .01 # remise à la valeur initiale car accélération de déplacement x dans le jeu au fil du temps
    ...
    def update(self, dt):
        ...
        if not self.state_game_over and self.state_game_has_started: # tant que pas GAME OVER et que le jeu a démarré, on permet l'avancement
            ...
            while self.current_offset_y >= spacing_y: # tant que le décallage dépasse ou est égal à une ligne (un écart entre deux lignes)
                ...
                self.SPEED *= (1 + self.ACCELERATION) # accélération à chaque tile
                self.SPEED_X *= (1 + self.ACCELERATION) # accélération à chaque tile
    ...
    def on_menu_button_pressed(self):
        if self.state_game_over: # si GAME OVER
            ...
            self.sound_gameover_voice.stop() #DEBUG car sinon le son GameOver n'est pas joué à chaque fois comme il est long
